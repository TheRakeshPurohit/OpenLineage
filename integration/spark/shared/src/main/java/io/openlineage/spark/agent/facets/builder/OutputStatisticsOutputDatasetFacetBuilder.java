...
public class OutputStatisticsOutputDatasetFacetBuilder
  ...
  protected void build(
    ...
    if (metrics.containsKey(Metric.WRITE_BYTES)
        || metrics.containsKey(Metric.WRITE_RECORDS)) {
      consumer.accept(
          "outputStatistics",
          context
              .getOpenLineage()
              .newOutputStatisticsOutputDatasetFacet(
                  Optional.of(metrics.get(Metric.WRITE_RECORDS))
                      .map(Number::longValue)
                      .orElse(null),
                  Optional.of(metrics.get(Metric.WRITE_BYTES))
                      .map(Number::longValue)
                      .orElse(null)));
    }
  }
