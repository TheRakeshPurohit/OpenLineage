...
from openlineage.airflow.extractors.dbapi_utils import get_table_schemas, get_datasets_from_schemas
...
class MySqlExtractor(BaseExtractor):
    ...
    def extract(self) -> TaskMetadata:
        ...
        schemas = get_table_schemas(
            self._get_hook(),
            self._information_schema_query(sql_meta.in_tables + sql_meta.out_tables)
        )

        inputs = get_datasets_from_schemas(database, source, sql_meta.in_tables, schemas)
        outputs = get_datasets_from_schemas(database, source, sql_meta.out_tables, schemas)
